  ...
  if (!enabled) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center px-4">
        ...
        <h3 className="reels-font-display font-bold text-white text-lg mb-2">Connect your store</h3>
        <p className="reels-font-body text-sm mb-6 max-w-md" style={{ color: 'var(--reels-text-muted)' }}>
          Set <code className="text-xs bg-black/30 px-1 rounded">VITE_DEV_VENDOR_SLUG</code> to your vendor store slug, or pass <code className="text-xs bg-black/30 px-1 rounded">vendorSlug</code> /{' '}
          <code className="text-xs bg-black/30 px-1 rounded">vendorId</code> from the portal shell, then reload.
        </p>
      </div>
    );
  }
  ...
  if (reels.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <h3 className="reels-font-display font-bold text-white text-lg mb-2">No reels yet</h3>
        ...
        <button type="button" onClick={onNewReel} className="reels-font-body text-sm font-semibold px-6 py-2.5 rounded-xl text-white" style={{ background: 'var(--reels-accent)' }}>
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <h2 className="reels-font-display font-bold text-2xl text-white">My Reels</h2>
        <button onClick={onNewReel} className="reels-font-body text-sm font-semibold px-4 py-2 rounded-xl text-white" style={{ background: 'var(--reels-accent)' }}>
          + New Reel
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
        {reels.map((reel, index) => (
          ...
          >
            {/* Thumbnail */}
            <div className="relative aspect-[9/16] overflow-hidden" style={{ background: 'var(--reels-surface)' }}>
            ...
            {/* Info */}
            <div className="p-3">
              <p className="reels-font-body text-xs text-white font-medium truncate mb-1">{reel.product.name}</p>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="relative">
                  ...
                  {menuOpen === reel.id && (
                    <div className="absolute right-0 top-6 z-10 rounded-lg py-1 min-w-[140px]" style={{ background: 'var(--reels-surface)', border: '1px solid var(--reels-glass-border)' }}>
                      {[
                        ...
                        <button
                          type="button"
                          key={action.label}
                          className="w-full flex items-center gap-2 px-3 py-2 reels-font-body text-xs hover:bg-white/5"
